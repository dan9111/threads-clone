...
export async function fetchFollowedPosts(userId: string, pageNumber = 1, pageSize = 20) {
  connectToDB();

  try {
    const currentUser = await User.findById(userId);
    const currentFollowedUsers = currentUser?.following || [];

    const skipAmount = (pageNumber - 1) * pageSize;

    const postsQuery = Thread.find({ parentId: { $in: [null, undefined] }, author: { $in: currentFollowedUsers } })
    .sort({ createdAt: "desc" })
    .skip(skipAmount)
    .limit(pageSize)
    .populate({
  ...
  return { posts, isNext };
    ...
    throw new Error(`Failed to fetch followed posts: ${error.message}`);
  }
}
...
export async function likeThread(threadId: string, userId: string) {
  ...
  try {
    ...
    if (!user || !thread) {
      return { success: false, message: 'User or Thread not found' };
    }
    ...
    if (hasLiked) {
      // Unlike the thread
      await User.updateOne({ _id: userId }, { $pull: { liked_threads: threadId } });
      await Thread.updateOne({ _id: threadId }, { $pull: { likedBy: userId } });

    } else {
      // Like the thread
      user.liked_threads.push(threadId);
